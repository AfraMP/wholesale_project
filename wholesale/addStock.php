...
<body>
	    ...
	    <ul>|
		   ...
		   <li class='active' onclick="location.href='addStock.php'">Add Stock</li>
...
<div class='container'>
	...
	<b>CATEGORIES</b>
		...
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method='get'>
			<input class='formInputItem' type=text name="catID" placeholder="Enter Category ID" required><input class='gBtn' type=submit name='submit' value='Go'>
		</form><br><br>
		<?php 
			if(isset($_GET['submit'])){
				...
				if($row<=0)
				{
					echo "<script>alert('no prducts available');</script>";
				}
				else{
					$sql="select * from products where category_id='$catID'";
				$result=mysqli_query($conn,$sql);
		    ...
		    <form action='updateStock.php' method='get'>
		    	<input class='formInputItem' type=text name='productID' placeholder='Enter Product ID' required><input class='formInputItem' type=text name='quantity' placeholder='Enter Quantity' required><input class='gBtn' type=submit name='submit' value='Update Stock'>
		    </form>
		<br>";
